...
post或者get发送的数据只能是以下两种类型不能是json字符串：
 ...
 - 序列化格式：k=v&k=v&k=v
 - json对象：{"k":v,"k":v}
	 - 发送数据类型是json对象时需要在Controller方法的参数中添加@RequestBody注解，解析json对象为model类
	 - 返回内容格式为json格式时需要在Controller方法中response参数或者返回值model类中添加@ResponseBody注解，解析model类为json对象
